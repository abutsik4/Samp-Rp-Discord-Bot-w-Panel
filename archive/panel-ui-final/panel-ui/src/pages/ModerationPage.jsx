    ...
    role_limits: Array.isArray(config?.role_limits)
      ? config.role_limits
          ...
          .map((entry) => ({
            ...
            role_name: entry.role_name ? String(entry.role_name) : "",
...
export function ModerationPage({ bot }) {
  ...
  const rolesQuery = useQuery(`${botApiBase}/roles?guildId=${encodeURIComponent(guildId || "")}`, {
    deps: [botKey, guildId],
    enabled: !!botKey && !!guildId,
  });
  ...
  const guildRoles = rolesQuery.data?.roles || [];
  ...
  function addRoleOverride() {
    ...
    setRateConfig((current) => {
      const next = normalizeRateConfig(current);
      const existingIndex = next.role_limits.findIndex((entry) => entry.role_id === roleId);
      const entry = { role_id: roleId, role_name: roleName, limit };

      if (existingIndex >= 0) {
        const roleLimits = [...next.role_limits];
        roleLimits[existingIndex] = entry;
        return { ...next, role_limits: roleLimits };
      }
      return { ...next, role_limits: [...next.role_limits, entry] };
    });

    setNewRoleOverrideId("");
    setNewRoleOverrideLimit(String(limit));
    setRateMessage("");
  }
  ...
  function updateRoleOverride(roleId, patch) {
    setRateConfig((current) => ({
      ...current,
      role_limits: current.role_limits.map((entry) =>
  ...
  function removeRoleOverride(roleId) {
    setRateConfig((current) => ({
      ...current,
      role_limits: current.role_limits.filter((entry) => entry.role_id !== roleId),
    }));
    setRateMessage("");
  }
          ...
          {selectedChannel ? (
            <>
              <SectionCard title="Role Overrides" icon={Shield}>
                <Alert type="info">
                  Set a consecutive-message limit for everyone with a given Discord role. When a user has multiple matching roles, the highest limit wins. Per-user overrides above still take priority over role overrides.
                </Alert>
                ...
                <div className="form-grid" style={{ marginTop: "0.75rem" }}>
                  <label>
                    Role
                    <select
                      value={newRoleOverrideId}
                      onChange={(e) => setNewRoleOverrideId(e.target.value)}
                      disabled={rolesQuery.loading}
                    >
                      <option value="">
                        {rolesQuery.loading ? "Loading roles…" : "Select role"}
                      </option>
                      {guildRoles
                  ...
                  <div className="row-actions" style={{ alignSelf: "end" }}>
                    <button className="btn--ghost btn--sm" type="button" onClick={addRoleOverride}>
                      <Plus size={13} />Add role limit
                    </button>
                  </div>
                </div>
                ...
                <div className="table-wrap" style={{ marginTop: "1rem" }}>
                  <table className="data-table">
                    ...
                    <tbody>
                      {rateConfig.role_limits.length > 0 ? rateConfig.role_limits.map((entry) => {
                        ...
                        return (
                          <tr key={entry.role_id}>
                            <td>{displayName}{live ? null : <span className="badge badge--danger" style={{ marginLeft: "0.5rem" }}>missing</span>}</td>
                            <td className="text-muted text-sm font-mono">{entry.role_id}</td>
                            <td style={{ width: "9rem" }}>
                              <input
                                type="number"
                                min="1"
                            ...
                            <td>
                              <button
                                className="btn--icon btn--danger-icon"
                                onClick={() => removeRoleOverride(entry.role_id)}
                                type="button"
                              >
                                <X size={13} />
                              </button>
                        ...
                        <tr>
                          <td colSpan={4} className="text-muted" style={{ padding: "18px", textAlign: "center" }}>
                            No role-specific limits configured for this channel.
                          </td>
                        </tr>
                      )}
                    </tbody>
