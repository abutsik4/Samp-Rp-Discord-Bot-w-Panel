        ...
        fields: [
          field(
            ...
            lines(
              ...
              "**`/quest`** — квест новичка (+$52k за 5 шагов)",
        ...
        fields: [
          field(
            ...
            lines(
              ...
              "**`/race user:@игрок bet:<$>`** — гонка «Три поворота» (вызов → принять/отказаться → 3 тактических выбора)",
          ...
          field(
            ...
            lines(
              ...
              "**`/bizstats id:<id>`** — подробные статы и состояние бизнеса",
              "**`/mbizstats id:<id>`** — быстрые статы по твоему бизнесу",
    ...
    embeds: [
      {
        ...
        fields: [
          ...
          field(
            "� Магазин и бусты",
            lines(
              "**`/shop`** — витрина с 6 категориями (титулы, цвета, флайры, покраска, скины, бусты)",
              "**`/mycollection`** — твои купленные предметы и что надето",
              "**`/equip id:<код>`** — надеть декоративный предмет",
              "**`/unequip category:<категория>`** — снять декоративный предмет",
              "Бусты (8 шт.) — постоянные бонусы к заработку, кулдаунам и /daily"
            )
          ),
          field(
            "🎟️ Лотерея и чёрный рынок",
            lines(
              "**`/lottery buy [count]`** — купить лотерейные билеты",
              "**`/lottery info`** — текущий статус лотереи",
              "**`/blackmarket browse`** — посмотреть чёрный рынок (4 товара/день)",
          ...
          field(
            "🕶️ Чёрный Рынок 2.0",
            lines(
              "**`/usejailpass`** — мгновенный выход из тюрьмы",
              "**`/userepairkit`** — восстановить состояние бизнеса на 30%",
              "**`/disguise`** — 60 мин защиты от PvP ограблений",
              "**`/hottip`** — случайный бонус (деньги/репа/розыск/ограбление)",
              "**`/secretheist`** — секретное ограбление (нужна карта)",
              "**`/wiretap user:@игрок`** — разведать баланс и статус цели",
              "**`/sabotage user:@игрок`** — саботировать следующую гонку цели",
              "**`/gangbmorder slot:<номер>`** — заказ для всей банды из казны",
              "Броня, аптечка, NOS — автоматически в дуэлях и гонках"
            )
          ),
        ],
